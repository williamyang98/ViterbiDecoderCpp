...
private:
    ...
    using BranchTable = ViterbiBranchTable<constraint_length,code_rate,int8_t>;
...
public:
    ...
    static sum_error_t update(Base& base, const int8_t* symbols, const size_t N, const BranchTable& table) {
        ...
        for (size_t s = 0; s < N; s+=Base::R) {
            ...
            bfly(base, &symbols[s], decision, old_metric, new_metric, table);
...
private:
    static void bfly(Base& base, const int8_t* symbols, decision_bits_t* decision, uint8_t* old_metric, uint8_t* new_metric, const BranchTable& table) {
        const __m256i* v_branch_table = reinterpret_cast<const __m256i*>(table.data());
